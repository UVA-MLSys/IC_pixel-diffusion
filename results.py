...
def cross_pspec(x, y, boxsize=1000.):
    ...
    # 🔍 Debug prints
    print("\n--- Cross Power Spectrum Debug Info ---")
    print("PS_xy (first 5):", PS_xy[:5])
    print("PS_xx (first 5):", PS_xx[:5])
    print("PS_yy (first 5):", PS_yy[:5])
    print("Any zeros in PS_xx?", np.any(PS_xx == 0))
    print("Any zeros in PS_yy?", np.any(PS_yy == 0))
...
def results(config, data_path):
  ...
  print("  std:", np.std(truth))

  print("observation:")
  print("  min:", np.min(observation))
  print("  max:", np.max(observation))
  print("  mean:", np.mean(observation))
  print("  std:", np.std(observation))

  print("samples (all combined):")
  ...
  np.save(os.path.join(data_path, 'cross_corr.npy'), samples_crosspspec)


  # Save power spectrum of generated samples
  np.save(os.path.join(data_path,'pspec.npy'), samples_pspec)
  np.save(os.path.join(data_path,'k.npy'), samples_k)
